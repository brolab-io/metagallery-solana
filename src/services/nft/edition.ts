...
export async function mintEditionFromMaster({
  ...
  const [originalEditionPDA] = (await PublicKey.findProgramAddress([
    Buffer.from('metadata', 'utf-8'),
    programPubkey.toBuffer(),
    masterPublicMint.toBuffer(),
    Buffer.from('edition', 'utf-8'),
  ], programPubkey));
  const [originalMetadataPDA] = (await PublicKey.findProgramAddress([
    Buffer.from('metadata', 'utf-8'),
    programPubkey.toBuffer(),
    masterPublicMint.toBuffer(),
  ], programPubkey));
  ...
  const [editionMarkPda] = (await PublicKey.findProgramAddress([
    Buffer.from('metadata', 'utf-8'),
    programPubkey.toBuffer(),
